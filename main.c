...
int Lookup_tb(unsigned int Adress)
{
	...
	while((i<trace_size) && (trace[i][ADRESS]!=Adress))
	 { i++;	
		if (i>nb_max_tb)
		{
			printf("\nTrace index Overflow! (i=%d ; loc_nb_tr=%d)\n",i,local_nb_tran);
...
void Run(FILE *f,unsigned int loop_exec, int quota, int threshold)
{
	FILE *fdat;
	int i;
	...
	while (1)
   {
      ...
      case 'I': next_line_is_adress = 1;														// In asm, next line is @
					 ...
					 if ( ((nb_tran % threshold) == 0) && (Sim_mode != '1') )
					 {
      ...
      case 'T': sscanf(line+22,"%x",&Read_Adress);											// Trace 
      			 ...
      			 i = Lookup_tb(Read_Adress);
					 if ((i<trace_size) && (trace[i][VALIDE])) 
					 		{tb_hit++;}
      			 trace[i][NB_EXEC]++;
...
int main(int argc, char **argv)
{
   ...
   switch(read_char) {
   	...
   	case '1': Run(f,loop_exec,quota,threshold);
   					break;
